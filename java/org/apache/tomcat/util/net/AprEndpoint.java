...
public class AprEndpoint extends AbstractEndpoint<Long> implements SNICallBack {
	...
	protected void createSSLContext(SSLHostConfig sslHostConfig) throws Exception {
		...
		if (openSslConf != null) {
			...
			try {
				...
				cctx = SSLConf.make(rootPool, SSL.SSL_CONF_FLAG_FILE | SSL.SSL_CONF_FLAG_SERVER
	...
	/*
	 * This method should only be called if there is no chance that the socket is
	 * currently being used by the Poller. It is generally a bad idea to call this
	 * directly from a known error condition.
	 */
	...
	public class Poller implements Runnable {
		...
		/**
		 * Variable poller timeout that adjusts depending on how many poll sets are in
		 * use so that the total poll time across all poll sets remains equal to
		 * pollTime.
		 */
		...
		/**
		 * Create the poller. With some versions of APR, the maximum poller size will be
		 * 62 (recompiling APR is necessary to remove this limitation).
		 */
		 ...
		 * @param flags
		 *            Events to poll for (Poll.APR_POLLIN and/or Poll.APR_POLLOUT)
		 */
